...
class Outer2 {					//외부 클래스
	...
	static class Inner {					//정적 내부 클래스
		...
		void inMethod() {
			...
			System.out.println("내부에서 외부 클래스의 정적멤버 변수 : sNum = "+sNum);
		}
	}
	static void outMethod() {					//외부 클래스의 정적 메소드
		...
		System.out.println("외부에서 외부 클래스의 정적멤버 변수 : sNum = "+sNum); 
	}
}
...
public class StaticInnerClassExam {
	public static void main(String[] args) {
		...
		inner.inMethod();
		//외부 클래스에서 외부 메소드는 모두 실행 가능하며, 내부 메소드 및 멤버 변수의 접근 및 실행이 불가능
